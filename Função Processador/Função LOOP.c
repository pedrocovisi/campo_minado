...
#define LOOP 7 // Define a nova inmstrução loop como 7
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case LOOP:
                    // Decrementamos o registrador
                    reg[rx]--;
                    
                    // Avalia se o registrador chegou a zero
                    if (reg[rx] != 0) {
                        // Se ainda não é zero pulamos para o endereço
                        selM1 = sPC; // Mux 1 seleciona PC
                        RW = 0;      // Lê memória
                        LoadPC = 1;  // Na próxima borda de subida de clock a memória é carregada e pulamos para o endereço que começa o loop
                    } else {
                        // Se for zero, o loop acabou. 
                        // Apenas incrementamos o PC para pular o endereço e ir para a próxima instrução.
                        IncPC = 1;
                    }
                    
                    // Volta para buscar a próxima instrução
                    state = STATE_FETCH;
                    break;
